...
public class UserDAO {
    ...
    public void addUser(User newUser, String profil) throws SQLException {
        PreparedStatement sql = getConnection()
                .prepareStatement("INSERT INTO User(login, password, profil) VALUES (?,?,?)");
    ...
    public void updateUser(User user, String profil, int id) throws SQLException {
        PreparedStatement sql = getConnection()
                .prepareStatement("UPDATE User SET login=?, password=?, profil=? WHERE id=? ");
    ...
    public Boolean verifyToken(String token) throws SQLException {
        PreparedStatement sql = getConnection()
                .prepareStatement("SELECT count(*) as nbToken FROM Token where tokenValue = ?");
        sql.setString(1, token);
